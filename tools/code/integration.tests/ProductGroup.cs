...
internal static class ProductGroup
{
    ...
    private static async ValueTask<FrozenDictionary<GroupName, ProductGroupDto>> GetApimResources(ProductName productName, ManagementServiceUri serviceUri, HttpPipeline pipeline, CancellationToken cancellationToken)
    {
    ...
    private static async ValueTask<FrozenDictionary<GroupName, ProductGroupDto>> GetFileResources(ProductName productName, ManagementServiceDirectory serviceDirectory, CancellationToken cancellationToken) =>
        await ProductGroupModule.ListInformationFiles(productName, serviceDirectory)
                                .ToAsyncEnumerable()
