...
public sealed record ApiDiagnosticsUri : IArtifactUri
{
    public Uri Uri { get; }

    public ApiDiagnosticsUri(ApiUri apiUri)
    {
        Uri = apiUri.AppendPath("diagnostics");
    }
}

public sealed record ApiDiagnosticsDirectory : IArtifactDirectory
{
    ...
    public ApiDiagnosticsDirectory(ApiDirectory apiDirectory)
    {
...
public sealed record ApiDiagnosticName
{
    private readonly string value;

    public ApiDiagnosticName(string value)
    {
        if (string.IsNullOrWhiteSpace(value))
        {
            throw new ArgumentException($"API diagnostic name cannot be null or whitespace.", nameof(value));
        }
...
public sealed record ApiDiagnosticUri : IArtifactUri
{
    public Uri Uri { get; }

    public ApiDiagnosticUri(ApiDiagnosticName apiDiagnosticName, ApiDiagnosticsUri apiDiagnosticsUri)
    {
        Uri = apiDiagnosticsUri.AppendPath(apiDiagnosticName.ToString());
    }
}

public sealed record ApiDiagnosticDirectory : IArtifactDirectory
{
    public ArtifactPath Path { get; }

    public ApiDiagnosticsDirectory ApiDiagnosticsDirectory { get; }

    public ApiDiagnosticDirectory(ApiDiagnosticName name, ApiDiagnosticsDirectory apiDiagnosticsDirectory)
    {
        Path = apiDiagnosticsDirectory.Path.Append(name.ToString());
        ApiDiagnosticsDirectory = apiDiagnosticsDirectory;
    }
}

public sealed record ApiDiagnosticInformationFile : IArtifactFile
{
    ...
    public ApiDiagnosticInformationFile(ApiDiagnosticDirectory diagnosticDirectory)
    {
        Path = diagnosticDirectory.Path.Append(Name);
        ApiDiagnosticDirectory = diagnosticDirectory;
    }
}

public sealed record ApiDiagnosticModel
{
    public required string Name { get; init; }

    public required ApiDiagnosticContractProperties Properties { get; init; }

    public sealed record ApiDiagnosticContractProperties
    {
        ...
        public static ApiDiagnosticContractProperties Deserialize(JsonObject jsonObject) =>
            new()
            {
    ...
    public static ApiDiagnosticModel Deserialize(ApiDiagnosticName name, JsonObject jsonObject) =>
        new()
        {
            Name = jsonObject.TryGetStringProperty("name") ?? name.ToString(),
            Properties = jsonObject.GetJsonObjectProperty("properties")
                                   .Map(ApiDiagnosticContractProperties.Deserialize)!
        };
}