...
| `SubscriptionModel` | Generate 0–5 subscriptions. |  |
...
| `ApiDiagnosticModel` | Build API x Logger pairs and pick a random subset. <br><br> Diagnostic name matches the logger name. <br><br> Generates `verbosity` and `logClientIp`. <br><br> For next state, regenerate from available APIs and loggers. | Skip `LoggerId` validation (format differs between APIM GET and extracted files). |
...
| `ProductGroupModel` | Pair all products with all groups and pick a random subset. | Validates DTO structure (`name` equals `Key.Name`, `properties.groupId` ends with `"groups/GroupName"`). <br><br> Publisher override set always empty. Links have nothing to override. |
| `ProductApiModel` | Pair all products with current API revisions and pick a random subset. | Only root/current revisions are linked (`api.Key.Name == api.RootName`). <br><br> Each API can be linked to at most one product. <br><br> Validates DTO structure (`name` equals `Key.Name`, `properties.apiId` ends with `"apis/ApiName"`). <br><br> Publisher override set always empty. Links have nothing to override. |
| `TagApiModel` | Pair all tags with current API revisions and pick a random subset. | Only root/current revisions are linked (`api.Key.Name == api.RootName`). <br><br> Each API can be linked to at most one tag. <br><br> Validates DTO structure (`name` equals `Key.Name`, `properties.apiId` ends with `"apis/ApiName"`). <br><br> Publisher override set always empty. Links have nothing to override. |
| `TagProductModel` | Pair all tags with all products and pick a random subset. | Each product can be linked to at most one tag. <br><br> Validates DTO structure (`name` equals `Key.Name`, `properties.productId` ends with `"products/ProductName"`). <br><br> Publisher override set always empty. Links have nothing to override. |
