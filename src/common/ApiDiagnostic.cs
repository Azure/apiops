...
public sealed record ApiDiagnosticResource : IResourceWithReference, IChildResource
{
    private ApiDiagnosticResource() { }
    ...
    public Type DtoType { get; } = typeof(ApiDiagnosticDto);
    ...
    public ImmutableDictionary<IResource, string> MandatoryReferencedResourceDtoProperties { get; } =
        ImmutableDictionary.Create<IResource, string>()
                           .Add(LoggerResource.Instance, nameof(ApiDiagnosticDto.Properties.LoggerId));

    public static ApiDiagnosticResource Instance { get; } = new();
}
