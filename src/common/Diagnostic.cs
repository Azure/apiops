...
public sealed record DiagnosticResource : IResourceWithReference
{
    private DiagnosticResource() { }
    ...
    public Type DtoType { get; } = typeof(DiagnosticDto);
    ...
    public ImmutableDictionary<IResource, string> MandatoryReferencedResourceDtoProperties { get; } =
        ImmutableDictionary.Create<IResource, string>()
                           .Add(LoggerResource.Instance, nameof(DiagnosticDto.Properties.LoggerId));

    public static DiagnosticResource Instance { get; } = new();
}
